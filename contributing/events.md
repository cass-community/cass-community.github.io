---
title: A Contributor's Guide for CASS Events
# excerpt: 
permalink: /contributing/events
layout: single
...
These screenshots illustrate how events are rendered on the CASS website (subject to change – hopefully the screenshots would also be updated). The annotations are meant to help connect the content that you see on the screen with the descriptions of the input that generates them, below.

### Event collection listing

{% include figure image_path="/assets/docs/2026-02-10-event-collection-annotated.jpeg" width="75%" class="align-center" popup=true %}

### Individual event pages

{% include figure image_path="/assets/docs/2026-02-10-event-page-annotated.jpeg" width="75%" class="align-center" popup=true %}
...
- **[Essential information](#essential-information)** includes the title, time and date, and the registration URL. There is also some metadata which should be included in the event, such as the "type" of event and "series" to which it belongs, and the CASS "activities" responsible for organizing the event.
- If the event is part of a larger conference or workshop, information about the **[hosting venue](#venue-information-optional)** should be provided.
- The **[people](#people-information-optional)** involved in the event, including presenters, panelists, moderators, organizers, as appropriate. It is also possible to provide brief bios for any of these people.
- **[Artifacts](#artifact-information-optional)** arising from the event. When we organize events, we have a strong desire that they should produce artifacts which can later be referenced by participants and others. Such artifacts should be included in the event file as they become available, which of course is generally after the event has occurred.
- The **[body](#event-body-optional)** of the event file generally provides the description or abstract of the event.

### Essential information

| Key | Presence | Description |
|-----|----------|-------------|
| `startdate` | REQUIRED | The starting date of the event, in ISO 8601 format ("YYYY-MM-DD"). |
| `enddate` | optional | The end date of the event, in ISO 8601 format ("YYYY-MM-DD"). The `enddate` is not needed for single-day events. |
| `time` | optional | The time frame for the event. Despite being technically optional, nearly all events will include a `time` entry. Exceptions would be multi-day events for which the time varies by day, or "umbrella" events which provide additional information about a group of events taking place in the same venue. |
| `registration_url` | optional | The url at which readers can register for the event, or which provides more information about the registration process. The field is expected for most upcoming events, but may be omitted if you're adding past events for completeness. If the event is hosted in a larger venue (i.e., a conference), it is probably the overall venue registration link. |
| `location` | REQUIRED | The location of the event.  See notes below for detailed guidance. |
| `type` | REQUIRED | The type or format of the event. We display the `type` near the title of the event in order to give the reader a better idea of what type of event they're looking at. You should generally try to use the same types as for other events on the site for consistency, but it is okay to create new types if none of the existing ones fit. |
| `series` | REQUIRED | To aid reader navigation and association of related events, we also associate each event with a `series`, which is usually a "branded" collection of events which are in some way related. `series` is intended to be distinct from `type`, though in some cases, all events in a series will be of the same type. The CASS site also displays pages listing the events for specific series, along with additional descriptive information based on the corresponding pages in the `_event-series` collection. In this way, readers can focus on a particular series of events which may interest them, or share links to a particular series with colleagues. | 
| `activities` | REQUIRED | The `activities` field is used to designate the CASS member organizations and/or working groups which have been involved in organizing the event. (The name of the field is a hold-over from terminology used on the IDEAS site.) The field may contain multiple entries. |
...
*Style notes:* `location`s will be one of three types, formatted as follows:
...
The CASS site currently uses the following `type`s
...
The CASS site currently uses the following `series`
...
| Key | Presence | Description |
|-----|----------|-------------|
| `venue` | REQUIRED | The is the name of the host event or institution. |
| `venue-url` | optional | A link to the hosting venue's website. If it is provided, the venue name will be rendered as a hyperlink. |
...
The `venue-url` should *not* be a link to the individual CASS event within the venue – that can be included in the `artifacts` field.
...
artifacts:
  - label: Slides
    doi: 10.6084/m9.figshare.c.7699772.v5
    note: (Figshare)
  - label: Conference program page
    url: https://meetings.siam.org/sess/dsp_programsess.cfm?SESSIONCODE=82327
    note: (part 1)
  - label: Conference program page
    url: https://meetings.siam.org/sess/dsp_programsess.cfm?SESSIONCODE=82328
    note: (part 2)
```
...
- `bsswt-event-label` for the event labels used on the BSSw tutorial site.  On the CASS site, this produces a link to the appropriate tutorial page on bssw-tutorial.github.io. Its real value is that on the BSSw tutorial site itself, artifacts of this type are recognized and ignored so that the tutorial page will not link to itself while allowing the same file to be used on other sites.
...
*Technical note:* for some related sites, for technical reasons, this content is in a `description` field in the frontmatter instead of in the body. If a `description` key is present, the CASS site will use its content in preference to the body content.

### Content for other sites

This section mentions briefly content you're likely to see in multi-purpose event files which is not used by the CASS site. The main point is that these are important for related sites and if they're present, you generally shouldn't fiddle with them.

| Key(s) | Used by |
|--------|---------|
| `venue-type` | BSSw tutorials |
| `organizer-ids` | BSSw tutorials |
| `deadlines` | BSSw tutorials |
| `helpers` | BSSw tutorials |
| `short-affil` and `github-id` in `presenters` | BSSw tutorials |
| `bsswio-topics` | IDEAS (HPC Best Practices webinars) |
| `qa-public-url` | IDEAS (HPC Best Practices webinars) |
| `survey-public-url` | IDEAS (HPC Best Practices webinars) |
| `parent-event-name` | IDEAS |
| `parent-event-file` | IDEAS |
| `header` | SWE-CSE BoFs |
| `title` and `url` in `presenters` | SWE-CSE BoFs |